...
func TestEvaluate_Approved(t *testing.T) {
	tests := []struct {
		...
		{"playwright test", "playwright test --config=playwright.config.ts", "playwright"},
		{"playwright test via node_modules", "./node_modules/.bin/playwright test --project=chromium", "node_modules/.bin+playwright"},
		{"vp test via node_modules", "./node_modules/.bin/vp test run src/components/Foo.test.ts", "node_modules/.bin+vp"},
		{"vp build via node_modules", "./node_modules/.bin/vp build --logLevel warn", "node_modules/.bin+vp"},
		...
		{"cmp", "cmp file1.bin file2.bin", "read-only"},
		{"comm", "comm -12 /tmp/a.txt /tmp/b.txt", "read-only"},
		{"comm -23", "comm -23 /tmp/a.txt /tmp/b.txt", "read-only"},
		{"tac", "tac file.txt", "read-only"},
		{"rev", "rev file.txt", "read-only"},
		{"column", "column -t file.tsv", "read-only"},
		{"nl", "nl file.txt", "read-only"},
		{"paste", "paste a.txt b.txt", "read-only"},
		{"join", "join a.txt b.txt", "read-only"},
		{"strings", "strings binary", "read-only"},
		{"bat", "bat file.txt", "read-only"},
		{"bc", `echo "1+1" | bc`, "echo | read-only"},
		{"dc", `echo "1 2 + p" | dc`, "echo | read-only"},
		{"expr", "expr 1 + 1", "read-only"},
		{"dig", "dig +short example.com", "read-only"},
		{"host", "host example.com", "read-only"},
		{"nslookup", "nslookup example.com", "read-only"},
		{"netstat", "netstat -tnlp", "read-only"},
		{"free", "free -h", "read-only"},
		{"nproc", "nproc", "read-only"},
		{"printenv", "printenv PATH", "read-only"},
		{"groups", "groups", "read-only"},
		{"xzcat", "xzcat file.xz", "read-only"},
		{"zcat", "zcat file.gz", "read-only"},
		{"b3sum", "b3sum file.bin", "read-only"},
		{"lsblk", "lsblk", "read-only"},
